...
# TSX ⇄ WAV directo
# ---------------------------------------------------------------------------
# No hace falta ninguna herramienta externa: TSX y WAV son dos formas
# distintas de representar la misma señal (una guarda los tonos como
# duraciones exactas en T-states, la otra como una forma de onda muestreada),
# y CAS es el punto intermedio natural entre ambas — ya está implementado y
# verificado por separado en cada dirección (WAV↔CAS en cas_tape.py,
# TSX↔CAS aquí mismo). Componer las dos conversiones es matemáticamente
# equivalente a una conversión directa TSX↔WAV, y reutiliza código ya
# probado en vez de duplicar la lógica de generación/lectura de audio.

def tsx_to_wav(tsx_data: bytes, baud: int = 1200, sample_rate: int = 44100,
               bit_depth: int = 8, pilot_seconds: float = 4.0,
               gaps: bool = True) -> bytes:
    """Convierte una cinta TSX a WAV, pasando por CAS internamente."""
    import cas_tape as ct
    cas = tsx_to_cas(tsx_data)
    return ct.cas_to_wav(cas, baud=baud, sample_rate=sample_rate,
                          bit_depth=bit_depth, pilot_seconds=pilot_seconds,
                          gaps=gaps)


def wav_to_tsx(wav_bytes: bytes, baud: int = 1200, pause_ms: int = 500) -> bytes:
    """Convierte una grabación WAV a TSX, pasando por CAS internamente.

    La velocidad se mide de la propia señal (igual que hace wav_to_cas): las
    cintas reales no van a velocidades redondas, así que no se impone un
    valor fijo.
    """
    import cas_tape as ct
    cas = ct.wav_to_cas(wav_bytes)
    return cas_to_tsx(cas, baud=baud, pause_ms=pause_ms)
