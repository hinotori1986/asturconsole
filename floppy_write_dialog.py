...
class FloppyWriteDialog(QDialog):
    def __init__(self, parent=None, image_path: str | None = None,
        ...
        # Aviso específico para los formatos "superformateados" (1600/800 KB)
        # cuando se va a FORMATEAR: el hueco entre sectores es una estimación,
        # no un valor medido en esta disquetera concreta (ver el docstring
        # de format_floppy en volumes.py). Al escribir sobre un disco ya
        # formateado esto no aplica, así que el aviso solo se muestra en
        # modo "formatear".
        self.aviso_superformato = QLabel()
        self.aviso_superformato.setWordWrap(True)
        self.aviso_superformato.setStyleSheet("color: #ffb454; font-size: 11px;")
        self.aviso_superformato.setVisible(False)
        lay.addWidget(self.aviso_superformato)

        def _actualizar_aviso_superformato():
            clave = self.fmt_combo.currentData()
            info = vol.FLOPPY_GEOMETRIES.get(clave, {})
            if modo == "formatear" and info.get("sufijo") is None:
                self.aviso_superformato.setText(
                    f"⚠ {info.get('etiqueta', clave)} es un formato superformateado: "
                    "el hueco entre sectores usado aquí es una estimación razonable, "
                    "no un valor medido en tu disquetera. Si el formateo da muchos "
                    "errores, prueba a formatear antes con la herramienta "
                    "«superformat» (paquete fdutils) y usa aquí solo la escritura."
                )
                self.aviso_superformato.setVisible(True)
            else:
                self.aviso_superformato.setVisible(False)

        self.fmt_combo.currentIndexChanged.connect(_actualizar_aviso_superformato)
        _actualizar_aviso_superformato()
        ...
        if modo == "escribir":
            ...
            if vol.IS_WINDOWS:
                # El formateo de pista a bajo nivel (IOCTL_DISK_FORMAT_TRACKS)
                # no está soportado por esta aplicación en Windows: su driver
                # de disquete tiene un soporte demasiado irregular como para
                # ofrecerlo con garantías. Se avisa aquí, antes de intentarlo,
                # en vez de dejar que falle al pulsar el botón.
                self.formato_radio = None
                nota_win = QLabel(
                    "En Windows no está disponible formatear cada pista antes de "
                    "grabar. Si el disquete da error de escritura, formatéalo "
                    "primero con el propio Windows (clic derecho sobre la unidad → "
                    "Formatear) y vuelve a intentarlo."
                )
                nota_win.setWordWrap(True)
                nota_win.setStyleSheet("color: #8892a8; font-size: 11px;")
                bl.addWidget(nota_win)
            else:
                ...
                nota = QLabel(
                    "Formatear cada pista es el equivalente a la opción /F de "
                    "COPIA720. Es bastante más lento, pero permite reutilizar "
                    "disquetes viejos, con otro formato o con la superficie "
                    "deteriorada, que de otro modo dan error de escritura."
                )
        ...
        # El formateo de disquete standalone no está soportado en Windows por
        # esta aplicación (ver format_floppy() en volumes.py); se avisa aquí,
        # antes de que el usuario intente pulsar el botón, y se deshabilita.
        self._formatear_no_disponible = vol.IS_WINDOWS and modo == "formatear"
        if self._formatear_no_disponible:
            aviso_win = QLabel(
                "El formateo de disquetes a bajo nivel no está disponible en "
                "Windows desde esta aplicación. Formatea el disquete con el "
                "propio Windows: en 'Este equipo', clic derecho sobre la unidad "
                "A: → Formatear."
            )
            aviso_win.setWordWrap(True)
            aviso_win.setStyleSheet("color: #ffb454; font-weight: 600;")
            lay.addWidget(aviso_win)
        ...
        if self._formatear_no_disponible:
            ...
            self.go_btn.setText("No disponible en Windows")
    ...
    def _empezar(self):
        ...
        formatear = (self._modo == "escribir" and self.formato_radio is not None
                    and self.formato_radio.isChecked())
        ...
        if not fallos:
            ...
            QMessageBox.warning(
                ...
                + (("\n\nPrueba a marcar «Formatear cada pista antes de grabarla», "
                    ...
                    if self.formato_radio is not None and not self.formato_radio.isChecked()
