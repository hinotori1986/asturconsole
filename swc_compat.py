...
def buscar(titulo: str, umbral: float = 0.82):
    ...
    for j in juegos:
        ...
        # Un juego y su secuela no son el mismo juego: si AMBOS mencionan
        # un número de entrega y no coinciden, se descartan por muy
        # parecidos que sean los títulos. Si la consulta no menciona
        # ninguno (habitual: el título interno de la ROM a veces es solo
        # el subtítulo, como "DIDDY'S KONG QUEST" sin el "2" de "Donkey
        # Kong Country 2"), no se descarta por esto — se deja decidir a
        # la comparación de texto de más abajo.
        num_candidato = _numero_secuela(candidato)
        if num_consulta and num_candidato and num_consulta != num_candidato:
            continue

        if candidato == consulta:
            return CompatEntry(j["n"], j["c"], j["o"]), 1.0
        # Coincidencia por prefijo o por sufijo: las cabeceras SNES
        # truncan a 21 caracteres (a veces cortando el final del título),
        # y otras veces el título interno es solo el subtítulo final del
        # nombre completo de la lista (el caso de Diddy's Kong Quest).
        if len(consulta) >= 8 and (candidato.startswith(consulta) or candidato.endswith(consulta)):
            ratio = 0.95
        else:
