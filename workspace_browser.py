...
class WorkspaceBrowser(QDialog):
    ...
    def __init__(self, parent=None, icon_dir: str | None = None):
        ...
        self.trabajar_btn.clicked.connect(
            lambda: self.abrir_carpeta.emit(self._actual))
    ...
    def _doble_clic(self, item: QListWidgetItem):
        ...
        if tipo == "dir":
            # Entrar en una carpeta abre directamente la ventana de trabajo con
            # sus archivos y las herramientas correspondientes, que es lo que
            # se quiere hacer casi siempre. Para solo mirar dentro está el
            # menú contextual.
            self.abrir_carpeta.emit(ruta)
            return
