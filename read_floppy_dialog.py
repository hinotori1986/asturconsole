...
class ReadFloppyDialog(QDialog):
    ...
    def _actualizar_nodo(self):
        ...
        if not dev:
            if vol.IS_WINDOWS:
                self.nodo_lbl.setText(
                    "No se detecta ninguna disquetera. Comprueba que está conectada "
                    "y que Windows la reconoce en 'Este equipo'."
                )
            else:
                self.nodo_lbl.setText(
        ...
        if vol.IS_WINDOWS:
            # En Windows no existe el concepto de "nodo con geometría"
            # separado (/dev/fd0u720 en Linux): la geometría se aplica al
            # leer el número de bytes correspondiente, no eligiendo un
            # dispositivo distinto, así que no hay nada más que avisar aquí.
            self.nodo_lbl.setText(f"Se leerá de: {dev}")
            return

        nodo = vol.floppy_device_for(dev, fmt)
        info_fmt = vol.FLOPPY_GEOMETRIES.get(fmt, {})
        if info_fmt.get("sufijo") is None:
            # Formatos sin nodo de dispositivo estándar (1600/800 KB): la
            # geometría se establece automáticamente con FDSETPRM justo
            # antes de leer, sobre el nodo genérico. No es un problema ni
            # requiere aviso, es el mecanismo normal para estos dos formatos.
            self.nodo_lbl.setText(
                f"Se leerá de {dev}, fijando la geometría de {fmt} KB "
                "automáticamente antes de empezar."
            )
        elif nodo == dev and fmt != "1440":
            self.nodo_lbl.setText(
                f"Se leerá de {dev}. AVISO: no existe el nodo con geometría "
                f"({dev}{info_fmt.get('sufijo', '')}), así que el sistema "
